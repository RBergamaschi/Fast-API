...
def list_activities(
    ...
    if data_inicial and not data_final:
        raise HTTPException(
            status_code=400,
            detail="Se data_inicial for fornecida, data_final também deve ser, ou virce e versa."
        )
    if data_inicial and data_final and data_final < data_inicial:
        raise HTTPException(
            status_code=400,
            detail="data_final deve ser maior que a data_inicial"
        )
    ...
    if not atividades:
        raise HTTPException(
            status_code=404,
            detail="Nenhuma atividade foi encontrada com os filtros fornecidos."
        )
    return atividades
